...
def Cost(chordProgression: list[float], idealDissonances: list[float], degree: int = 3) -> float:
    ...
    #these coefficients can be tweeked to place higher emphasis on different terms of the cost function
    dissonanceCoeffecient = 1
    distanceCoeffecient = 0.000000001
    sameNotePenaltyCoeffecient = 10
    ...
    #penalizes notes within a chord being to close together, prevents chords from being multiple overlayed notes of the same frequency
    minimumDistance = 0.05
    for i in range(len(idealDissonances)):
        chord = [chordProgression[i*degree + j] for j in range(degree)]
        for interval in permutations(chord, 2):
